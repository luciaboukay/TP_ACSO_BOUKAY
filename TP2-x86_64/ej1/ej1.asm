...
.node_loop:
    ...
    ; Check type match - CRITICAL FIX: Compare bytes directly
    mov al, byte [r15+16]  ; current->type
    cmp al, r12b           ; compare as bytes
    jne .next_node
    ...
    ; Concatenate: r14 = str_concat(r14, current->hash)
...
.end_loop:
    ...
    ; Concatenate prefix: r14 = str_concat(prefix, r14)
    mov rdi, r13        ; prefix
